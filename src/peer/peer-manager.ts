...
import type { PeerInfo, StoredPeerInfo, PeerState, PeerManagerConfig } from './types';
...
export class PeerManager extends EventEmitter<PeerManagerEvents> {
  ...
  private async handleIncomingConnection(connection: PeerConnection): Promise<void> {
    const nodeId = connection.peerId;
  ...
  private async startSyncSession(connection: PeerConnection, peer: PeerInfo): Promise<void> {
  ...
  private handleSyncError(nodeId: string): void {
    ...
    this.logger.info(`Reconnecting to ${nodeId} in ${delay}ms (attempt ${attempts})`);
  ...
  private async loadPeers(): Promise<void> {
    try {
      ...
      if (data) {
        const stored: StoredPeerInfo[] = JSON.parse(new TextDecoder().decode(data));
  ...
  private async savePeers(): Promise<void> {
    const stored: StoredPeerInfo[] = Array.from(this.peers.values()).map((p) => ({
      nodeId: p.nodeId,
      name: p.name,
      ticket: p.ticket,
