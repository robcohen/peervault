...
export class FileHistoryModal extends Modal {
  ...
  private renderVersionList(container: HTMLElement): void {
    ...
    // Current version
    const currentSection = section.createDiv({ cls: 'peervault-version-section' });
    currentSection.createEl('h3', { text: 'Current Version' });
    this.renderVersionItem(currentSection, this.versions[0]!, true);

    // Previous versions
    if (this.versions.length > 1) {
      const historySection = section.createDiv({ cls: 'peervault-version-section' });
  ...
  private renderVersionItem(container: HTMLElement, version: FileVersion, isCurrent: boolean): void {
  ...
  private async viewVersion(version: FileVersion): Promise<void> {
    ...
    try {
      ...
      if (content) {
        ...
        const modal = new VersionViewModal(this.app, this.filePath, version, content);
        modal.open();
      }
    } catch (error) {
  ...
  private async restoreVersion(version: FileVersion): Promise<void> {
    ...
    try {
      ...
      new Notice(`Restoring to version from ${new Date(version.timestamp).toLocaleString()}...`);
      ...
      new Notice('Version restored (simulated - full restore requires Loro time-travel integration)');
    } catch (error) {
