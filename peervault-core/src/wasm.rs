...
///
/// This sends our version vector, receives peer's version, exchanges updates.
async fn run_initiator_sync(
    peer_id: &str,
    stream: &mut IrohStream,
    loro_store: &LoroStore,
    on_event: &Option<Function>,
    pairing_nonce: Option<String>,
    device_name: Option<String>,
) -> Result<(), JsValue> {
    web_sys::console::log_1(&JsValue::from_str("[WASM] INITIATOR: starting sync protocol"));

    // Sync protocol:
    // 0. Send pairing handshake, receive response
    // 1. Send our version vector
    // 2. Receive peer's version vector
    // 3. Send updates since peer's version
    // 4. Receive updates from peer

    // Step 0: Send pairing handshake
    let handshake = PairingHandshake {
        nonce: pairing_nonce.clone(),
        device_name: device_name.clone(),
    };
    web_sys::console::log_1(&JsValue::from_str(&format!(
        "[WASM] INITIATOR Step 0: sending pairing handshake, nonce={}",
        pairing_nonce.as_deref().map(|n| &n[..16.min(n.len())]).unwrap_or("none")
    )));
    stream.send(&serde_json::to_vec(&handshake).unwrap_or_default()).await
        .map_err(|e| JsValue::from_str(&format!("Failed to send handshake: {}", e)))?;

    // Receive pairing response
    let response_data = stream.recv().await
        .map_err(|e| JsValue::from_str(&format!("Failed to receive pairing response: {}", e)))?;
    let response: PairingResponse = serde_json::from_slice(&response_data)
        .map_err(|e| JsValue::from_str(&format!("Invalid pairing response: {}", e)))?;

    if !response.accepted {
        let error_msg = response.error.unwrap_or_else(|| "Pairing rejected".into());
        web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] INITIATOR Step 0: pairing rejected: {}", error_msg)));
        return Err(JsValue::from_str(&format!("Pairing rejected: {}", error_msg)));
    }
    web_sys::console::log_1(&JsValue::from_str("[WASM] INITIATOR Step 0: pairing accepted"));

    // Get our version vector
    let our_vv = loro_store.version_vector();
    web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] INITIATOR Step 1: sending VV (len={})", our_vv.len())));

    // Send version vector length + data
    stream.send(&our_vv).await
        .map_err(|e| JsValue::from_str(&format!("Failed to send version: {}", e)))?;
    web_sys::console::log_1(&JsValue::from_str("[WASM] INITIATOR Step 1: VV sent"));

    // Receive peer's version vector
    web_sys::console::log_1(&JsValue::from_str("[WASM] INITIATOR Step 2: waiting for peer VV..."));
    let peer_vv = stream.recv().await
        .map_err(|e| JsValue::from_str(&format!("Failed to receive version: {}", e)))?;
    web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] INITIATOR Step 2: received peer VV (len={})", peer_vv.len())));

    // Export updates since peer's version
    let updates = loro_store.export_updates(Some(&peer_vv))
        .map_err(|e| JsValue::from_str(&format!("Failed to export updates: {}", e)))?;
    web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] INITIATOR Step 3: exporting updates (len={})", updates.len())));

    // Send our updates
    stream.send(&updates).await
        .map_err(|e| JsValue::from_str(&format!("Failed to send updates: {}", e)))?;
    web_sys::console::log_1(&JsValue::from_str("[WASM] INITIATOR Step 3: updates sent"));

    // Receive peer's updates
    web_sys::console::log_1(&JsValue::from_str("[WASM] INITIATOR Step 4: waiting for peer updates..."));
    let peer_updates = stream.recv().await
        .map_err(|e| JsValue::from_str(&format!("Failed to receive updates: {}", e)))?;
    web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] INITIATOR Step 4: received peer updates (len={})", peer_updates.len())));

    // Import peer's updates
    if !peer_updates.is_empty() {
        web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] INITIATOR Step 4: importing {} bytes...", peer_updates.len())));
        loro_store.import_updates(&peer_updates)
            .map_err(|e| JsValue::from_str(&format!("Failed to import updates: {}", e)))?;

        // Verify import worked
        let files_after = loro_store.list_files(None)
            .map_err(|e| JsValue::from_str(&format!("Failed to list after import: {}", e)))?;
        web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] INITIATOR Step 4: imported successfully, now have {} files", files_after.len())));

        info!(peer_id = %peer_id, peer_updates_len = peer_updates.len(), "[INITIATOR] Step 4: Imported peer updates successfully");
    } else {
        info!(peer_id = %peer_id, "[INITIATOR] Step 4: No peer updates to import (empty)");
    }
    ...
    if let Some(ref callback) = on_event {
        let event = serde_json::json!({
            ...
            "peer_id": peer_id,
            "direction": "outgoing",
            "updates_received": peer_updates.len(),
            "updates_sent": updates.len()
        });
        let _ = callback.call1(&JsValue::NULL, &JsValue::from_str(&event.to_string()));
    }

    info!(peer_id = %peer_id, "Sync complete");
    Ok(())
}

/// Pairing handshake message (sent by initiator)
#[derive(Debug, serde::Serialize, serde::Deserialize)]
struct PairingHandshake {
    /// One-time pairing nonce (optional, only for new pairing)
    nonce: Option<String>,
    /// Peer's device name
    device_name: Option<String>,
}

/// Pairing response (sent by acceptor)
#[derive(Debug, serde::Serialize, serde::Deserialize)]
...
///
/// This accepts streams from the connection and runs the sync protocol
/// as the acceptor (responding to initiator's requests).
async fn handle_incoming_streams(
    ...
    web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] Stream handler started for peer {}", &peer_id[..16])));

    loop {
        // Get the connection
        let connections_guard = connections.read().await;
        let conn = match connections_guard.get(&peer_id) {
            Some(c) => c,
            None => {
                debug!(peer_id = %peer_id, "Connection not found, stopping stream handler");
                break;
            }
        };
        ...
        if conn.is_closed() {
            web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] Stream handler: connection closed for {}", &peer_id[..16])));
            break;
        }

        // Try to accept a stream
        web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] Stream handler: waiting for stream from {}...", &peer_id[..16])));
        match conn.accept_stream().await {
            Ok(mut stream) => {
                web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] Stream handler: accepted stream from {}", &peer_id[..16])));

                // Get the store
                let store_guard = store.read().await;
                let loro_store = match store_guard.as_ref() {
                    Some(s) => s,
                    None => {
                        error!(peer_id = %peer_id, "Store not initialized");
                        continue;
                    }
                };

                // Run sync as acceptor:
                // 0. Receive and validate pairing handshake
                // 1. Receive peer's version vector
                // 2. Send our version vector
                // 3. Receive peer's updates
                // 4. Send our updates since peer's version

                web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR: starting sync protocol for peer {}", &peer_id[..16])));

                // Step 0: Receive pairing handshake
                web_sys::console::log_1(&JsValue::from_str("[WASM] ACCEPTOR Step 0: waiting for pairing handshake..."));
                let handshake_data = match stream.recv().await {
                    Ok(data) => data,
                    Err(e) => {
                        web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 0 FAILED: {}", e)));
                        continue;
                    }
                };

                let handshake: PairingHandshake = match serde_json::from_slice(&handshake_data) {
                    Ok(h) => h,
                    Err(e) => {
                        web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 0: invalid handshake JSON: {}", e)));
                        // Send rejection
                        let response = PairingResponse { accepted: false, error: Some("Invalid handshake format".into()) };
                        let _ = stream.send(&serde_json::to_vec(&response).unwrap_or_default()).await;
                        continue;
                    }
                };

                web_sys::console::log_1(&JsValue::from_str(&format!(
                    "[WASM] ACCEPTOR Step 0: received handshake, nonce={}, device={}",
                    handshake.nonce.as_deref().map(|n| &n[..16.min(n.len())]).unwrap_or("none"),
                    handshake.device_name.as_deref().unwrap_or("unknown")
                )));

                // Validate pairing
                let is_known = known_peers.read().await.contains_key(&peer_id);

                if is_known {
                    web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 0: peer {} is already known, accepting", &peer_id[..16])));
                } else {
                    // Unknown peer - must have valid nonce
                    match &handshake.nonce {
                        Some(nonce) => {
                            ...
                            if let Some(&expires_at) = pending.get(nonce) {
                                ...
                                if now > expires_at {
                                    // Nonce expired
                                    pending.remove(nonce);
                                    web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 0: pairing nonce expired")));
                                    let response = PairingResponse { accepted: false, error: Some("Pairing nonce expired".into()) };
                                    let _ = stream.send(&serde_json::to_vec(&response).unwrap_or_default()).await;
                                    continue;
                                }
                                // Valid nonce - consume it and add peer
                                pending.remove(nonce);
                                drop(pending); // Release write lock
                                known_peers.write().await.insert(peer_id.clone(), now);
                                web_sys::console::log_1(&JsValue::from_str(&format!(
                                    "[WASM] ACCEPTOR Step 0: pairing nonce valid, auto-added peer {}",
                                    &peer_id[..16]
                                )));
                                ...
                                // Unknown nonce
                                web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 0: unknown pairing nonce")));
                                let response = PairingResponse { accepted: false, error: Some("Invalid pairing nonce".into()) };
                                let _ = stream.send(&serde_json::to_vec(&response).unwrap_or_default()).await;
                                continue;
                            }
                        }
                        None => {
                            // No nonce and unknown peer - reject
                            web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 0: unknown peer without pairing nonce")));
                            let response = PairingResponse { accepted: false, error: Some("Pairing required - unknown peer".into()) };
                            let _ = stream.send(&serde_json::to_vec(&response).unwrap_or_default()).await;
                            continue;
                        }
                    }
                ...
                // Send acceptance
                let response = PairingResponse { accepted: true, error: None };
                if let Err(e) = stream.send(&serde_json::to_vec(&response).unwrap_or_default()).await {
                    web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 0: failed to send acceptance: {}", e)));
                    continue;
                }
                web_sys::console::log_1(&JsValue::from_str("[WASM] ACCEPTOR Step 0: pairing accepted"));

                // Receive peer's version vector
                web_sys::console::log_1(&JsValue::from_str("[WASM] ACCEPTOR Step 1: waiting for peer VV..."));
                let peer_vv = match stream.recv().await {
                    Ok(vv) => vv,
                    Err(e) => {
                        web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 1 FAILED: {}", e)));
                        continue;
                    }
                };
                web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 1: received peer VV (len={})", peer_vv.len())));

                // Send our version vector
                let our_vv = loro_store.version_vector();
                web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 2: sending our VV (len={})", our_vv.len())));
                if let Err(e) = stream.send(&our_vv).await {
                    web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 2 FAILED: {}", e)));
                    continue;
                }
                web_sys::console::log_1(&JsValue::from_str("[WASM] ACCEPTOR Step 2: VV sent"));

                // Receive peer's updates
                web_sys::console::log_1(&JsValue::from_str("[WASM] ACCEPTOR Step 3: waiting for peer updates..."));
                let peer_updates = match stream.recv().await {
                    Ok(updates) => updates,
                    Err(e) => {
                        web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 3 FAILED: {}", e)));
                        continue;
                    }
                };
                web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 3: received peer updates (len={})", peer_updates.len())));

                // Import peer's updates
                if !peer_updates.is_empty() {
                    if let Err(e) = loro_store.import_updates(&peer_updates) {
                        web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 3 FAILED import: {}", e)));
                        continue;
                    }
                    web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 3: imported {} bytes", peer_updates.len())));
                } else {
                    web_sys::console::log_1(&JsValue::from_str("[WASM] ACCEPTOR Step 3: no peer updates to import"));
                }

                // Export and send our updates since peer's version
                let our_updates = match loro_store.export_updates(Some(&peer_vv)) {
                    Ok(updates) => updates,
                    Err(e) => {
                        web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 4 FAILED export: {}", e)));
                        continue;
                    }
                };
                web_sys::console::log_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 4: exporting our updates (len={})", our_updates.len())));

                if let Err(e) = stream.send(&our_updates).await {
                    web_sys::console::error_1(&JsValue::from_str(&format!("[WASM] ACCEPTOR Step 4 FAILED send: {}", e)));
                    continue;
                }
                web_sys::console::log_1(&JsValue::from_str("[WASM] ACCEPTOR Step 4: updates sent"));
                ...
                if let Some(ref callback) = on_event {
                    let event = serde_json::json!({
                        ...
                        "direction": "incoming",
                        "updates_received": peer_updates.len(),
                        "updates_sent": our_updates.len()
                    });
                    let _ = callback.call1(&JsValue::NULL, &JsValue::from_str(&event.to_string()));
                }

                info!(peer_id = %peer_id, "Sync complete (acceptor)");
            }
            Err(e) => {
                let err_str = format!("{}", e);
                if err_str.contains("closed") || err_str.contains("ConnectionLost") {
                    debug!(peer_id = %peer_id, "Connection closed, stopping stream handler");
                    break;
                }
                warn!(peer_id = %peer_id, error = %e, "Failed to accept stream");
        ...
        // Release locks between iterations
        drop(connections_guard);
    }

    // Remove connection when done
    connections.write().await.remove(&peer_id);
    info!(peer_id = %peer_id, "Stream handler stopped, connection removed");
}
